...
if (/forceWebm/.test(src)) fail("0.5.31 F3:vc=webm 死参数须删(0.5.28 自愈梯残留,overlay 零消费;两闸门曾编码相反事实)");
if (!src.includes(".[0-9]{13}" + "\\.mp3$/")) fail("0.5.31 🟡-2:evict 须过滤在途 tmp(钉代码本体字符串)");
if (!/retryPath/.test(src) || !/noStream/.test(src)) fail("0.5.31 🟡-2/🟡-3:cache-miss 重试/负缓存确定性缺失");
